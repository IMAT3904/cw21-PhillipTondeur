...
namespace Engine
{
	...
	{
		enum Region { None = -1, Vertex = 0, Fragment, Geometry, TesselationControl, TessellationEvaluation, Compute };
		...
		{
			Log::error("Could not open shader vertex source: {0}", filepath);
			return;
		}
		handle.close();
